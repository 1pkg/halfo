...
Architector::provide()
{
	...
	figure->view()->body()->setVelocity(
		cocos2d::Vec2(
			(_side ? -1 : 1) * cocos2d::RandomHelper::random_real<float>(
				Master::instance().metric().absolute(LINEAR_SPEED_LIMIT.first),
				Master::instance().metric().absolute(LINEAR_SPEED_LIMIT.second)
			),
			0.0f
		)
	);
	figure->view()->body()->setAngularVelocity(
		cocos2d::RandomHelper::random_real<float>(
			Master::instance().metric().absolute(ANGULAR_SPEED_LIMIT.first),
			Master::instance().metric().absolute(ANGULAR_SPEED_LIMIT.second)
		)
	);
	figure->view()->setPosition(Master::instance().metric().spawn(_side));
	_side = !_side;
	return figure;
}
