...
Architector::provide()
{
	...
	figure->view()->body()->setVelocity(
		cocos2d::Vec2(
			(_side ? -1 : 1) * cocos2d::RandomHelper::random_real<float>(
				Master::instance().get<Components::Metric>("metric").absolute(LINEAR_SPEED_LIMIT.first),
				Master::instance().get<Components::Metric>("metric").absolute(LINEAR_SPEED_LIMIT.second)
			),
			0.0f
		)
	);
	figure->view()->body()->setAngularVelocity(
		cocos2d::RandomHelper::random_real<float>(
			Master::instance().get<Components::Metric>("metric").absolute(ANGULAR_SPEED_LIMIT.first),
			Master::instance().get<Components::Metric>("metric").absolute(ANGULAR_SPEED_LIMIT.second)
		)
	);
	figure->view()->setPosition(Master::instance().get<Components::Metric>("metric").spawn(_side));
	_side = !_side;
	return figure;
}
...
Architector::fill(unsigned int count)
{
	for (size_t i = 0; i < count;)
	{
		...
		if (object->area() < Master::instance().get<Components::Metric>("metric").absolute2(LIMIT_AREA))
			continue;
